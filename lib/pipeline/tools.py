...
def detect_segment_track(
    imgfiles, out_path, thresh=0.1, min_size=None, device="cuda", save_vos=True
):
    ...
    """
    # People detector
    detector = YOLO("yolov11x_people_detector.engine")
    ...
    for t, imgpath in enumerate(tqdm(imgfiles)):
        ...
        with torch.no_grad():
            with torch.amp.autocast("cuda"):
                detections = detector.predict(img_cv2, verbose=False, conf=thresh)[0]
                boxes = detections.boxes.data.cpu().numpy()[:, :5]
                if not len(boxes):
        ...
        ### --- DEVA ---
        if len(boxes) > 0 and (boxes[:, -1] > thresh).sum() > 0:
            track_valid = boxes[:, -1] > thresh  # only use high-confident
